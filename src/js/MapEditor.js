...
function mapEditor(divID, loadXML, notAllowEdit, onlyMap) {
  ...
  var scriptConfig = {
    ...
    onMoveObstacleMessage:
      'Move the obstacle to the game field for place it, or out of game field to remove.',
  ...
  var MapEditorMove = {
    pos: function (event, currentHoldObj, refObj) {
      var xpoint =
        event.pageX - refObj.offset().left - currentHoldObj.width / 2;
      var ypoint =
        event.pageY - refObj.offset().top - currentHoldObj.height / 2;
      return {
        x: xpoint,
        y: ypoint,
      };
  ...
  var self = {
    ...
    loadXML: function (xmlDoc) {
      ...
      if (
        mapMetaData.difficuly == NaN ||
        mapMetaData.difficuly <= 0 ||
        mapMetaData.difficuly >= 5
      ) {
      ...
      rootDoc
        ...
        .find('obstacles')
        .each(function () {
          MapEditorObstacleList.add(
            parseInt($(this).attr('x')),
            parseInt($(this).attr('y')),
            parseInt($(this).attr('r'))
          );
        });
      ...
      for (var x in playerListDir) {
        ...
        if (
          playerListDirItem.r == playerR &&
          playerListDirItem.angle == playerA
        ) {
          mapPlayerPoint = playerListDirItem;
          mapPlayerPoint.posX =
            parseInt(playerDoc.attr('x')) - mapPlayerPoint.offsetX;
          mapPlayerPoint.posY =
            parseInt(playerDoc.attr('y')) - mapPlayerPoint.offsetY;
        }
      }
    },
    ...
    makeUI: function (mainDiv, onlyEditor) {
      if (!onlyEditor) {
        ...
        var gameWindowOut = self.make.div(
          45,
          45,
          300,
          400,
    ...
    makeConfigUI: function (putIntoDiv) {
      ...
      level.find(':empty').append(
        self.make.inputSelect(
          [
            {
              val: '1',
              text: 'Easy',
      ...
      speed.find(':empty').append(
        self.make.inputText(
          mapMetaData.game_speed,
          function () {
      ...
      self.hover(
        growth,
        'Set how much the snake will growth for each collect apple.'
      );
      growth.find(':empty').append(
        self.make.inputText(
          mapMetaData.growth,
          function () {
            var newValue = $(this).val();
      ...
      descriptionText.find(':empty').append(
        self.make.textArea(
    ...
    makeToolBarUI: function (putIntoDiv) {
      ...
      var obstacleTrTd1 = $('<td></td>').append(
        self.make
          .image(scriptConfig.obstacle, 0, 0, 30, 30)
          .mousedown(selectSize, self.onMoveObstacle)
      ...
      var playerTrTd1 = $('<td></td>').append(
        self.make
          .image(scriptConfig.player, 0, 0, 20, 40)
          .mousedown(playerAngle, self.onMovePlayer)
          .css('position', 'relative')
      );
      ...
      var submitSave = $('<input></input>')
        .val(scriptConfig.submitSave)
        .click('save', self.submit)
        .attr('type', 'button');
      var submitSavePublic = $('<input></input>')
        .val(scriptConfig.submitPublic)
        .click('public', function (event) {
    ...
    makeMap: function (gameWindow) {
      ...
      for (var i = 0; i < list.length; i++) {
        var r = list[i].r;
        var r2 = r * 2;
        var image = self.make.image(
          scriptConfig.obstacle,
          list[i].x - r,
          list[i].y - r,
          r2,
          r2
      ...
      if (mapPlayerPoint) {
        var playerPoint = self.make.image(
          mapPlayerPoint.url,
          mapPlayerPoint.posX,
          mapPlayerPoint.posY,
          mapPlayerPoint.x,
    ...
    onMoveObstacleAttac: function (newObj, width, heigth, radius) {
      newObj.mousedown(function () {
        if (currentHold == null && allow_edit) {
          MapEditorObstacleList.remove(
            $(this).position().left + radius,
            $(this).position().top + radius,
            radius
          );
          $(this).appendTo(mainDiv);
          var offsetX =
            gameWindow.offset().left -
            mainDiv.offset().left +
            $(this).position().left;
          var offsetY =
            gameWindow.offset().top -
            mainDiv.offset().top +
            $(this).position().top;
          currentHold = {
            obj: $(this),
    ...
    submitXML: function (event) {
      ...
      outHTML +=
        '\t<description>' + mapMetaData.description + '</description>\n';
      outHTML += '\t<difficuly>' + mapMetaData.difficuly + '</difficuly>\n';
      outHTML += '\t<map x="300" y="400" />\n';
      outHTML +=
        '\t<gamespeed type="simple">' +
        mapMetaData.game_speed +
        '</gamespeed>\n';
      outHTML +=
        '\t<growthspeed type="simple">' +
        mapMetaData.growth +
        '</growthspeed>\n';
      ...
      if (mapPlayerPoint) {
        outHTML +=
          '\t<player x="' +
          Math.floor(mapPlayerPoint.posX) +
          '" y="' +
          Math.floor(mapPlayerPoint.posY) +
          '" r="' +
          Math.floor(mapPlayerPoint.r) +
          '" a="' +
          Math.floor(mapPlayerPoint.angle) +
          '" s="4" />\n';
      } else {
      ...
      for (var x in obstacleBuilderList) {
        ...
        outHTML +=
          '\t\t<item x="' +
          Math.floor(obstacleBuilderListItem.x) +
          '" y="' +
          Math.floor(obstacleBuilderListItem.y) +
          '" r="' +
          Math.floor(obstacleBuilderListItem.r) +
          '" />\n';
      }
      outHTML += '\t</obstacles>\n';
      outHTML += '</snakeappmap>\n';
